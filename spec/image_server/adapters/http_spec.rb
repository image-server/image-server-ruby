...
RSpec.describe ImageServer::Adapters::Http do
  ...
  let(:uploader) { ImageServer::Adapters::Http.new('p', 'http://example.com/image.url?a=1&b=2') }
  let(:response) { {hash: 'IMAGEHASH', width: '20', height: '40'} }
  let(:upload_uri) { URI.parse('http://example.com') }
  ...
  describe '#upload' do
    ...
    context 'the url does not contain scheme' do
      let(:uploader) { ImageServer::Adapters::Http.new('p', '//example.com/image.url?a=1&b=2') }

      it 'requests the image with http' do
        uploader.upload(upload_uri)
      end
    end
    ...
    context 'the source is a file' do
      ...
      let(:uploader) { ImageServer::Adapters::Http.new('p', image) }
